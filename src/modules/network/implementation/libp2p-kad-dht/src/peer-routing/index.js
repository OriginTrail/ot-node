...
module.exports = (dht) => {
    ...
    const closerPeersSingle = async (key, peer) => {
        dht._log(
            `closerPeersSingle ${uint8ArrayToString(key, 'base32')} from ${peer.toB58String()}`,
        );
    ...
    const getPublicKeyFromNode = async (peer) => {
        ...
        if (!msg.record || !msg.record.value) {
            throw errcode(
                new Error(`Node not responding with its public key: ${peer.toB58String()}`),
                'ERR_INVALID_RECORD',
            );
        }
        ...
        if (!recPeer.equals(peer)) {
            throw errcode(
                new Error('public key does not match id'),
                'ERR_PUBLIC_KEY_DOES_NOT_MATCH_ID',
            );
        }
    ...
    return {
        ...
        async getPublicKey(peer) {
            ...
            const addrs = ((peerData && peerData.addresses) || []).map(
