...
class WorkerQueue {
    ...
    stop(err) {
        ...
        this.queue.clear();
        this.log(
            'worker:stop, %d workers still running',
            this.run.workers.filter((w) => w.running).length,
        );
    ...
    fill() {
        ...
        while (
            this.queue.pending + this.queue.size < this.concurrency &&
