...
class RoutingTable {
            ...
            refreshCpls.map(async (lastRefresh, index) => {
                try {
                    ...
                    if (this._numPeersForCpl(prefixLength) === 0) {
                        ...
                        for (let n = index + 1; n < lastCpl + 1; n += 1) {
                            try {
                                // eslint-disable-next-line no-await-in-loop
                                await this._refreshCommonPrefixLength(
                                    n,
                                    lastRefresh,
                                    force === true,
                                );
    ...
    async _refreshCommonPrefixLength(cpl, lastRefresh, force) {
        ...
        log(
            `starting refreshing cpl ${cpl} with key ${peerId.toB58String()} (routing table size was ${this.kb.count()})`,
        );
        ...
        log(
            `finished refreshing cpl ${cpl} with key ${peerId.toB58String()} (routing table size was ${this.kb.count()})`,
        );
    }
    ...
    _getTrackedCommonPrefixLengthsForRefresh(maxCommonPrefix) {
        ...
        for (let i = 0; i <= maxCommonPrefix; i += 1) {
    ...
    async _generateRandomPeerId(targetCommonPrefixLength) {
        ...
        const key = await this._makePeerId(
            this.kb.localNodeId,
            randomUint16,
    ...
    _maxCommonPrefix() {
        ...
        for (const len of this._prefixLengths()) {
            if (len > prefixLength) {
                prefixLength = len;
            }
        }
