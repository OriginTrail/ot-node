...
class ServiceAgreementsMetadataMigration extends BaseMigration {
    ...
    async executeMigration() {
        ...
        const identities = {};
        // for each asset
        for (const { ual, keyword } of assetsMetadata) {
            const { blockchain, contract, tokenId } = this.ualService(ual);
            ...
            // eslint-disable-next-line no-await-in-loop
            await this.updateTables(blockchain, contract, tokenId, keyword, identities[blockchain]);
        }
    }

    async updateTables(blockchain, contract, tokenId, keyword, identityId) {
