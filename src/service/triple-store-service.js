...
class TripleStoreService {
    ...
    findResourceId(assertion) {
        for (const nquad of assertion) {
            ...
            let subjectFoundInObject = false;
            for (const innerNquad of assertion) {
                const innerQuads = innerNquad.split(' ');
                const innerObject = innerQuads[2];
                if (innerObject.includes(subject)) {
                    subjectFoundInObject = true;
                    break;
                }
            }
