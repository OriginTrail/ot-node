...
describe('JWT token decoding', async () => {
    it('decodes JWT if valid token is provided', async () => {
        const token = jwtUtil.generateJWT(uuid());
        expect(jwtUtil.decodeJWT(token)).to.be.ok;
    });

    it('returns null if invalid token is provided', async () => {
        for (const val of nonJwts) {
            expect(jwtUtil.decodeJWT(val)).to.be.null;
        }
    });
});
