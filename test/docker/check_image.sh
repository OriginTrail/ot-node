...
docker exec mynode /bin/sh -c "curl -X POST http://127.0.0.1:8900/api/latest/import -F standard_id=GS1-EPCIS -F file=@/ot-node/current/importers/xml_examples/Retail/03_Pink_to_orange_shipment.xml" > $TRAVIS_BUILD_DIR/importResult.json
