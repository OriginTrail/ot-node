	...
	socket.on('event', function(data){ 
		...
		if(data.request == 'trail-request')
		{
			...
			product.getTrailByQuery(queryObject, function(trail_graph){
	        	socket.emit('event', {response: 'trail-response', responseData: trail_graph, clientRequest: clientRequest});
	     	});
		}
		else if(data.request == 'import-request')
		{
	        ...
	        importer.importXML(input_file, selected_importer, function(data){
	            socket.emit('event', {response: 'import-response', responseData: data, clientRequest: clientRequest})
	        });
		}
		else if(data.request == 'blockchain-request')
		{
	        ...
	        blockchain.getFingerprint(owner, batch_uid_hash, function(data){
	            socket.emit('event', {response: 'blockchain-response', responseData: data, clientRequest: clientRequest})
	        });
		}
		else if(data.request == 'expiration-request')
		{
		    ...
		    product.getExpirationDates(queryObject, function(data){
		        socket.emit('event', {response: 'expiration-response', responseData: data, clientRequest: clientRequest})
		    });
		}
		else
		{
			socket.emit('event', {response: 'Unsupported request'}); 
		}
	});
});
