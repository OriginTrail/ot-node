...
contract('Offer testing', async (accounts) => {
    ...
    it('Should test token transfers for holding', async () => {
        ...
        let res = await profileStorage.profile.call(DC_identity);
        ...
        for (i = 0; i < 3; i += 1) {
            assert(initialStake[i].add(tokenAmountPerHolder).eq(res[i].stake), `Stake amount incorrect for account ${i}`);
            assert((new BN(0)).eq(res[i].stakeReserved), `Stake amout incorrect for account ${i}`);
        }
        res = await profileStorage.profile.call(DC_identity);
        ...
        promises = [];
        for (i = 0; i < 3; i += 1) {
            promises[i] = holdingStorage.getHolderPaidAmount.call(offerId, identities[i]);
        }
        res = await Promise.all(promises);
        for (i = 0; i < 3; i += 1) {
            assert(tokenAmountPerHolder.eq(res[i]), `Paid amount incorrect for account ${i}!`
                + `Got ${res[i].toString()}, but expected ${tokenAmountPerHolder.toString()}!`);
        }
    });
