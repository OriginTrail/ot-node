...
class RpcController {
    ...
    initializeNetworkApi() {
        ...
        this.networkService.handleMessage('/store', (result) =>
            this.publishService.handleStore(result),
        );

        this.networkService.handleMessage('/resolve', (result) =>
        ...
        this.networkService.handleMessage('/search/result', (result) =>
        ...
        this.networkService.handleMessage('/search/assertions/result', (result) =>
            this.queryService.handleSearchAssertionsResult(result),
        );
    ...
    initializeServiceApi() {
        ...
        this.app.post(
            ...
            async (req, res, next) => {
                ...
                if (!_.isString(req.body.query) || !_.isString(req.body.type)) {
                    return next({ code: 400, message: 'All params must be of type String' });
                }
                ...
                if (!allowedQueries.includes(req.query.type.toLowerCase())) {
                    const errMsg = `Unallowed query type, currently supported types: ${allowedQueries.join(
                        ', ',
                    )}`;
    ...
    async publish(req, res, next, options) {
        ...
        if (
            req.body &&
            req.body.data &&
            Buffer.byteLength(req.body.data, 'utf-8') > constants.MAX_FILE_SIZE
        ) {
            return next({
                code: 400,
                message: `File size limit is ${constants.MAX_FILE_SIZE / (1024 * 1024)}MB.`,
