...
class OtJsonImporter {
        ...
        vertices.filter(vertex => (vertex.vertexType === constants.vertexType.entityObject
            ...
            .forEach((entityVertex) => {
                ...
                edges.filter(edge => (edge.edgeType === constants.edgeType.identifierRelation))
                    .forEach((edge) => {
                        vertices.filter(vertices => vertices._id === edge._to)
                ...
                edges.filter(edge => (edge.edgeType === constants.edgeType.otRelation))
                    .forEach((edge) => {
                        ...
                        const id = (vertices.filter(vertex => vertex._id === edge._to)[0]).uid;
    ...
    async getImportedOtObject(datasetId, objectIndex, color = null) {
        ...
        let result = await this.db.findDocumentsByImportIdAndOtObjectId(datasetId, otObjectId);
        // result = result[0];
        if (!result.vertices || result.vertices.length === 0) {
            throw Error(`Unnable to find vertices for importId: ${datasetId} and objectId: ${otObjectId}`);
        }
        if (!result.edges || result.edges.length === 0) {
            throw Error(`Unnable to find edges for importId: ${datasetId} and objectId: ${otObjectId}`);
        }

        for (const vertex of result.vertices) {
            if (vertex.vertexType === constants.vertexType.data
                && vertex.data != null) {
                vertex.data = vertex.encrypted[color];
            }
        }

        for (const edge of result.edges) {
            if (edge.edgeType === constants.edgeType.otRelation
                && edge.properties != null) {
                edge.properties = edge.encrypted[color];
            }
        }

        const otObject = await this._createObjectGraph(otObjectId, result.vertices, result.edges);
        return otObject;
    }
